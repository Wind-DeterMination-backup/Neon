...
Neon 是一个 Mindustry 纯客户端工具集（Java 模组），将以下 13 个模组合并为一个安装包：
...
如果你只想安装一次、一次性获得这十三类常用“信息叠加 + 操作效率 + 数据查询 + 自动更新”功能，Neon 会更省事。

提示：Neon 已包含上述 13 个模块的功能，建议不要与它们的独立版本同时启用，避免重复功能或 UI 冲突。
...
Neon is a client-side Mindustry toolkit (Java mod) that bundles thirteen modules into one package:
...
Note: Neon already contains these thirteen modules. Do not enable standalone versions at the same time, or you may get duplicate UI and feature conflicts.
